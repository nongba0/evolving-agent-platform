...
3. **동적 계획 (Planner)** — 오케스트레이터가 **사용자 프롬프트를 LLM으로 분석**해 1~5개의 하위 작업(sub-task)으로 분해. 생성된 계획은 체크포인트에 저장돼 재개 시 재계획 없이 재사용됩니다. *(하드코딩 템플릿이 아니라 입력마다 동적으로 생성)*
4. **노드 실행 (순차 컨텍스트 체인)** — 계획된 노드를 순서대로 실행하며, **앞 단계의 출력 + 원본 작업을 다음 노드 입력으로 주입**. LLM 네트워크 호출은 `asyncio.to_thread`로 워커 스레드에 오프로드해 **이벤트 루프(API 서버)를 막지 않습니다**.
5. **L1 마이크로 검증** — 각 노드는 최대 **3회 재시도**. 빈 응답·스키마 위반 시 스스로 재시도하고, 끝내 실패하면 Task를 `failed`로.
6. **비용 추적 & 예산 상한** — 모든 호출의 input/output 토큰과 예상 비용(USD)을 `token_costs` 테이블에 기록. 누적 비용이 **상한($0.50 기본)을 넘으면 Task를 `paused`로 자동 정지**.
7. **체크포인트** — 노드가 성공할 때마다 출력 상태를 `checkpoints`에 저장. 재개 시 완료된 노드는 **건너뛰고** 멈춘 지점부터 이어서 실행(`POST /api/tasks/{id}/resume`).
8. **가시화** — 대시보드가 status·logs·costs 엔드포인트를 폴링해 실시간으로 진행 상황과 누적 비용을 보여줌.
...
핵심 인프라(비용·체크포인트·L1 검증)와 **동적 파이프라인**이 구현됐고, 다음을 고도화 중입니다.

- [x] **동적 파이프라인 생성** — 사용자 프롬프트를 LLM Planner로 분석해 Sub-task를 동적으로 분할 *(하드코딩 템플릿 제거 완료)*
- [x] **논블로킹 LLM 호출** — `asyncio.to_thread`로 블로킹 호출을 오프로드해 이벤트 루프 정지 제거
- [ ] **L2 Milestone Gateway** — 분기점에서 결과물의 의미론적 품질을 검증하는 단계 추가
- [ ] **병렬 DAG 스케줄링** — 독립적인 노드를 의존성 그래프 기반으로 동시 실행 (현재는 순차 컨텍스트 체인)
